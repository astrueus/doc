...
func (c *AccountController) referer() string {
	...
	if u == "" {
		u = config.URLFor("HomeController.Index")
	}
	return u
}
...
func (c *AccountController) Login() {
	...
	if c.Ctx.Input.IsPost() {
		...
		member, err := model.NewMember().Login(account, password)
		if err == nil {
			...
			if strings.EqualFold(isRemember, "yes") {
				...
				v, err := gob.Encode(remember)
				if err == nil {
					c.SetSecureCookie(config.GetAppKey(), "login", v, time.Now().Add(time.Hour*24*30).Unix())
				}
			}
...
func (c *AccountController) DingTalkLogin() {
	...
	member, err := model.NewMember().TmpLogin(tmpReader)
	if err == nil {
		member.LastLoginTime = time.Now()
		_ = member.Update("last_login_time")
...
func (c *AccountController) Register() {
	...
	if c.Ctx.Input.IsPost() {
		...
		member.Role = config.MemberGeneralRole
...
func (c *AccountController) FindPassword() {
	...
	if c.Ctx.Input.IsPost() {
		...
		member, err := model.NewMember().FindByFieldFirst("email", email)
		if err != nil {
		...
		if member == nil || member.AuthMethod == config.AuthMethodLDAP {
			c.JsonResult(6011, i18n.Tr(c.Lang, "message.account_not_support_retrieval"))
		}

		count, err := model.NewMemberToken().FindSendCount(email, time.Now().Add(-1*time.Hour), time.Now())
		...
		memberToken := model.NewMemberToken()

		memberToken.Token = string(krand.Krand(32, krand.KC_RAND_KIND_ALL))
		...
		data := map[string]any{
			...
			"url":       config.URLFor("AccountController.FindPassword", "token", memberToken.Token, "mail", email),
			"BaseUrl":   c.BaseUrl(),
		}
		...
		go func(mailConf *config.SmtpConf, email string, body string) {
		...
		c.JsonResult(0, "ok", config.URLFor("AccountController.Login"))
	}
	...
	if token != "" && email != "" {
		memberToken, err := model.NewMemberToken().FindByFieldFirst("token", token)
		if err != nil {
			logs.Error(err)
...
func (c *AccountController) ValidEmail() {
	...
	member, err := model.NewMember().Find(memberToken.MemberId)
	if err != nil {
		logs.Error(err)
		c.JsonResult(6005, i18n.Tr(c.Lang, "message.user_not_existed"))
	}
	hash, err := passutil.PasswordHash(password1)
	if err != nil {
		logs.Error(err)
...
func (c *AccountController) Logout() {
	c.SetMember(model.Member{})
	c.SetSecureCookie(config.GetAppKey(), "login", "", -3600)
	u := c.Ctx.Request.Header.Get("Referer")
	c.Redirect(config.URLFor("AccountController.Login", "url", u), 302)
}
...
func (c *AccountController) Captcha() {
	...
	_ = c.SetSession(config.CaptchaSessionName, txt)
