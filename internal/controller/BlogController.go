...
func (c *BlogController) Index() {
	...
	if blog.BlogExcerpt == "" {
		c.Data["Description"] = htmlutil.AutoSummary(blog.BlogRelease, 120)
	} else {
		c.Data["Description"] = blog.BlogExcerpt
	}

	if nextBlog, err := model.NewBlog().QueryNext(blogId); err == nil {
		c.Data["Next"] = nextBlog
	}
	if preBlog, err := model.NewBlog().QueryPrevious(blogId); err == nil {
...
func (c *BlogController) List() {
	...
	blogList, totalCount, err = model.NewBlog().FindToPager(pageIndex, config.PageSize, 0, "")
	...
	if totalCount > 0 {
		...
		for _, blog := range blogList {
			//如果没有添加文章摘要，则自动提取
			if blog.BlogExcerpt == "" {
				blog.BlogExcerpt = htmlutil.AutoSummary(blog.BlogRelease, 120)
			}
			blog.Link()
		}
	} else {
...
func (c *BlogController) ManageList() {
	...
	blogList, totalCount, err := model.NewBlog().FindToPager(pageIndex, config.PageSize, c.Member.MemberId, "")
...
func (c *BlogController) ManageSetting() {
	...
	if c.Ctx.Input.IsPost() {
		...
		if blogType == 1 {
			...
			doc, err := model.NewDocument().FindByIdentityFirst(documentIdentify, book.BookId)
			if err != nil {
		...
		if blogId > 0 {
			if c.Member.IsAdministrator() {
				blog, err = model.NewBlog().Find(blogId)
			} else {
				blog, err = model.NewBlog().FindByIdAndMemberId(blogId, c.Member.MemberId)
			}
			...
			if blogIdentify != "" {
				...
				if b, err := model.NewBlog().FindByIdentify(blogIdentify); err == nil && b.BlogId != blogId {
			...
			//如果设置了文章标识
			if blogIdentify != "" {
				if model.NewBlog().IsExist(blogIdentify) {
					c.JsonResult(6004, i18n.Tr(c.Lang, "message.blog_id_existed"))
				}
			}
...
func (c *BlogController) ManageEdit() {
	...
	if c.Ctx.Input.IsPost() {
		...
		//如果是关联文章，需要同步关联的文档
		if blog.BlogType == 1 {
			doc, err := model.NewDocument().Find(blog.DocumentId)
			if err != nil {
...
func (c *BlogController) Upload() {
	...
	// 如果是超级管理员，则不判断权限
	if c.Member.IsAdministrator() {
		_, err := model.NewBlog().Find(blogId)
		...
		_, err := model.NewBlog().FindByIdAndMemberId(blogId, c.Member.MemberId)
	...
	filePath := filepath.Join(config.WorkingDirectory, "uploads", "blog", time.Now().Format("200601"), fileName+ext)
	...
	if strings.EqualFold(ext, ".jpg") || strings.EqualFold(ext, ".jpeg") || strings.EqualFold(ext, ".png") || strings.EqualFold(ext, ".gif") {
		httpPath = "/" + strings.Replace(strings.TrimPrefix(filePath, config.WorkingDirectory), "\\", "/", -1)
		if strings.HasPrefix(httpPath, "//") {
			httpPath = config.URLForWithCdnImage(string(httpPath[1:]))
		}
	} else {
		...
		if attachment.HttpPath == "" {
			attachment.HttpPath = config.URLForNotHost("BlogController.Download", ":id", blogId, ":attach_id", attachment.AttachmentId)
...
func (c *BlogController) RemoveAttachment() {
	...
	if !c.Member.IsAdministrator() {
		_, err := model.NewBlog().FindByIdAndMemberId(attach.DocumentId, c.Member.MemberId)
