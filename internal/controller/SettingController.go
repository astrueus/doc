...
func (c *SettingController) Password() {
	...
	if c.Ctx.Input.IsPost() {
		if c.Member.AuthMethod == config.AuthMethodLDAP {
...
func (c *SettingController) Upload() {
	...
	filePath := filepath.Join(config.WorkingDirectory, "uploads", time.Now().Format("200601"), fileName+ext)
	...
	if member, err := model.NewMember().Find(c.Member.MemberId); err == nil {
		...
		if err == nil {
			if strings.HasPrefix(avater, "/uploads/") {
				os.Remove(filepath.Join(config.WorkingDirectory, avater))
			}
			c.SetMember(*member)
		} else {
