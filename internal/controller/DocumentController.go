...
import (
	...
	"git.itopcms.com/jackliu/doc/pkg/gopool"
...
func (c *DocumentController) Index() {
	...
	if bookResult.IsUseFirstDocument {
		...
		if err == nil {
			...
			c.Data["Description"] = htmlutil.AutoSummary(doc.Release, 120)
	...
	tree, err := model.NewDocument().CreateDocumentTreeForHtml(bookResult.BookId, selected)
...
func (c *DocumentController) Read() {
	...
	attach, err := model.NewAttachment().FindListByDocumentId(doc.DocumentId)
	if err == nil {
	...
	tree, err := model.NewDocument().CreateDocumentTreeForHtml(bookResult.BookId, doc.DocumentId)
...
func (c *DocumentController) Edit() {
	...
	if c.Member.IsAdministrator() {
		...
		if bookResult.RoleId == config.BookObserver {
			c.JsonResult(6002, i18n.Tr(c.Lang, "message.item_not_exist_or_no_permit"))
		}
	}
	...
	trees, err := model.NewDocument().FindDocumentTree(bookResult.BookId)
...
func (c *DocumentController) Upload() {
	...
	if config.GetUploadFileSize() > 0 && moreFile.Size > config.GetUploadFileSize() {
	...
	//如果文件类型设置为 * 标识不限制文件类型
	if config.IsAllowUploadFileExt(ext) == false {
	...
	if c.Member.IsAdministrator() {
		...
		// 如果没有编辑权限
		if book.RoleId != config.BookEditor && book.RoleId != config.BookAdmin && book.RoleId != config.BookFounder {
			c.JsonResult(6006, i18n.Tr(c.Lang, "message.no_permission"))
		}
	...
	filePath := filepath.Join(config.WorkingDirectory, "uploads", identify)
	...
	attachment.FilePath = strings.TrimPrefix(filePath, config.WorkingDirectory)
	...
	if filetil.IsImageExt(moreFile.Filename) {
		attachment.HttpPath = "/" + strings.Replace(strings.TrimPrefix(filePath, config.WorkingDirectory), "\\", "/", -1)
		if strings.HasPrefix(attachment.HttpPath, "//") {
			attachment.HttpPath = config.URLForWithCdnImage(string(attachment.HttpPath[1:]))
		}
	...
	if attachment.HttpPath == "" {
		attachment.HttpPath = config.URLForNotHost("DocumentController.DownloadAttachment", ":key", identify, ":attach_id", attachment.AttachmentId)
...
func (c *DocumentController) DownloadAttachment() {
	...
	if err != nil {
		...
		// 如果不是超级管理员则判断权限
		if c.Member == nil || c.Member.Role != config.MemberSuperRole {
	...
	// 查找附件
	attachment, err := model.NewAttachment().Find(attachId)
	...
	c.Ctx.Output.Download(filepath.Join(config.WorkingDirectory, attachment.FilePath), attachment.FileName)
	c.StopRun()
}
...
func (c *DocumentController) RemoveAttachment() {
	...
	document, err := model.NewDocument().Find(attach.DocumentId)
	...
	if c.Member.Role != config.MemberSuperRole {
		rel, err := model.NewRelationship().FindByBookIdAndMemberId(document.BookId, c.Member.MemberId)
		if err != nil {
			logs.Error(err)
			c.JsonResult(6004, i18n.Tr(c.Lang, "message.no_permission"))
		}

		if rel.RoleId == config.BookObserver {
			c.JsonResult(6004, i18n.Tr(c.Lang, "message.no_permission"))
		}
	}
	...
	os.Remove(filepath.Join(config.WorkingDirectory, attach.FilePath))
...
func (c *DocumentController) Content() {
	...
	if c.Ctx.Input.IsPost() {
		...
		go func(history *model.DocumentHistory) {
...
func (c *DocumentController) Export() {
	...
	if !strings.HasPrefix(bookResult.Cover, "http:://") && !strings.HasPrefix(bookResult.Cover, "https:://") {
		bookResult.Cover = config.URLForWithCdnImage(bookResult.Cover)
	}
	...
	outputPath := filepath.Join(config.GetExportOutputPath(), strconv.Itoa(bookResult.BookId))
...
func (c *DocumentController) QrCode() {
	...
	uri := config.URLFor("DocumentController.Index", ":key", identify)
...
func (c *DocumentController) DeleteHistory() {
	...
	err = model.NewDocumentHistory().Delete(historyId, docId)
	if err != nil {
		logs.Error(err)
...
func (c *DocumentController) RestoreHistory() {
	...
	err = model.NewDocumentHistory().Restore(historyId, docId, c.Member.MemberId)
	if err != nil {
		logs.Error(err)
		c.JsonResult(6002, i18n.Tr(c.Lang, "message.failed"))
	}
...
func (c *DocumentController) Compare() {
	...
	history, err := model.NewDocumentHistory().Find(historyId)
	if err != nil {
		logs.Error("DocumentController.Compare => ", err)
		c.ShowErrorPage(60003, err.Error())
	}

	doc, err := model.NewDocument().Find(history.DocumentId)
