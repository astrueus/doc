...
func (c *BookController) Index() {
	...
	if itemsets, err := model.NewItemsets().First(1); err == nil {
...
func (c *BookController) SaveBook() {
	...
	book, err := model.NewBook().Find(bookResult.BookId)
...
func (c *BookController) PrivatelyOwned() {
	...
	//只有创始人才能变更私有状态
	if bookResult.RoleId != config.BookFounder {
...
func (c *BookController) Transfer() {
	...
	err = model.NewRelationship().Transfer(bookResult.BookId, c.Member.MemberId, member.MemberId)
...
func (c *BookController) UploadCover() {
	...
	filePath = filepath.Join(config.WorkingDirectory, "uploads", time.Now().Format("200601"), fileName+"_small"+ext)
	...
	url := "/" + strings.Replace(strings.TrimPrefix(filePath, config.WorkingDirectory), "\\", "/", -1)
	...
	book.Cover = config.URLForWithCdnImage(url)

	if err := book.Update(); err != nil {
		c.JsonResult(6001, "保存图片失败")
	}
	//如果原封面不是默认封面则删除
	if oldCover != config.GetDefaultCover() {
		os.Remove("." + oldCover)
	}
...
func (c *BookController) Import() {
	...
	if books, _ := model.NewBook().FindByField("identify", identify, "book_id"); len(books) > 0 {
		c.JsonResult(6006, i18n.Tr(c.Lang, "message.project_id_existed"))
	}
...
func (c *BookController) Delete() {
	...
	err = model.NewBook().ThoroughDeleteBook(bookResult.BookId)
...
func (c *BookController) Release() {
	...
	if c.Member.IsAdministrator() {
		...
		if book.RoleId != config.BookAdmin && book.RoleId != config.BookFounder && book.RoleId != config.BookEditor {
			c.JsonResult(6003, i18n.Tr(c.Lang, "message.no_permission"))
		}
		bookId = book.BookId
	}
	go model.NewBook().ReleaseContent(bookId, c.Lang)
...
func (c *BookController) SaveSort() {
	...
	for _, item := range docs {
		if docId, ok := item["id"].(float64); ok {
			doc, err := model.NewDocument().Find(int(docId))
			if err != nil {
				logs.Error(err)
				continue
			}
			...
			if parentId > 0 {
				if parent, err := model.NewDocument().Find(int(parentId)); err != nil || parent.BookId != bookId {
					continue
				}
			}
...
func (c *BookController) Team() {
	...
	members, totalCount, err := model.NewTeamRelationship().FindByBookToPager(book.BookId, pageIndex, config.PageSize)
...
func (c *BookController) TeamAdd() {
	...
	_, err = model.NewTeam().First(teamId, "team_id")
	if err != nil {
		if err == orm.ErrNoRows {
	...
	if _, err := model.NewTeamRelationship().FindByBookId(book.BookId, teamId); err == nil {
...
func (c *BookController) TeamDelete() {
	...
	err = model.NewTeamRelationship().DeleteByBookId(book.BookId, teamId)
...
func (c *BookController) TeamSearch() {
	...
	searchResult, err := model.NewTeamRelationship().FindNotJoinBookByBookIdentify(book.BookId, keyword, 10)
...
func (c *BookController) ItemsetsSearch() {
	...
	searchResult, err := model.NewItemsets().FindItemsetsByName(keyword, 10)
