...
func (c *TemplateController) isPermission() error {
	...
	if !c.Member.IsAdministrator() {
		...
		book, err := model.NewBook().FindByIdentify(bookIdentify, "book_id")
		if err != nil {
			if err == orm.ErrNoRows {
				return errors.New("项目不存在或没有权限")
			}
...
func (c *TemplateController) Get() {
	...
	template, err := model.NewTemplate().Find(templateId)
...
func (c *TemplateController) List() {
	...
	templateList, err := model.NewTemplate().FindAllByBookId(c.BookId)
...
func (c *TemplateController) Add() {
	...
	if c.Member.IsAdministrator() {
		...
		template.IsGlobal = 0
		//如果不是管理员需要判断是否有项目权限
		rel, err := model.NewRelationship().FindByBookIdAndMemberId(c.BookId, c.Member.MemberId)
		if err != nil || rel.RoleId == config.BookObserver {
			c.JsonResult(403, "没有权限")
		}
		//如果修改的模板不是本人创建的，并且又不是项目创建者则禁止修改
		if template.MemberId > 0 && template.MemberId != c.Member.MemberId && rel.RoleId != config.BookFounder {
			c.JsonResult(403, "没有权限")
		}
	}
...
func (c *TemplateController) Delete() {
	...
	if c.Member.IsAdministrator() {
		err := model.NewTemplate().Delete(templateId, 0)
		if err != nil {
			c.JsonResult(500, "删除失败")
		}
	} else {
		//如果不是管理员需要判断是否有项目权限
		rel, err := model.NewRelationship().FindByBookIdAndMemberId(template.BookId, c.Member.MemberId)
		if err != nil || rel.RoleId == config.BookObserver {
			c.JsonResult(403, "没有权限")
		}

		//如果是创始人或管理者可以直接删除模板
		if rel.RoleId == config.BookFounder || rel.RoleId == config.BookAdmin {
			err := model.NewTemplate().Delete(templateId, 0)
			if err != nil {
				c.JsonResult(500, "删除失败")
			}
		...
		if err := model.NewTemplate().Delete(templateId, c.Member.MemberId); err != nil {
			c.JsonResult(500, "删除失败")
		}
	}
	c.JsonResult(0, "OK")
