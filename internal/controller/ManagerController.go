...
func (c *ManagerController) Index() {
	...
	c.Data["Model"] = model.NewDashboard().Query()
	c.Data["Action"] = "index"
}
...
func (c *ManagerController) Users() {
	...
	members, totalCount, err := model.NewMember().FindToPager(pageIndex, config.PageSize)
	if err != nil {
	...
	if totalCount > 0 {
		...
		for _, item := range members {
			item.Avatar = config.URLForWithCdnImage(item.Avatar)
		}
	} else {
		c.Data["PageHtml"] = ""
	}
...
func (c *ManagerController) CreateMember() {
	...
	if ok, err := regexp.MatchString(config.RegexpAccount, account); account == "" || !ok || err != nil {
	...
	if ok, err := regexp.MatchString(config.RegexpEmail, email); !ok || err != nil || email == "" {
	...
	member.Role = config.SystemRole(role)
	member.Avatar = config.GetDefaultAvatar()
...
func (c *ManagerController) UpdateMemberStatus() {
	...
	if member.Role == config.MemberSuperRole {
...
func (c *ManagerController) ChangeMemberRole() {
	...
	if role != int(config.MemberAdminRole) && role != int(config.MemberGeneralRole) {
...
func (c *ManagerController) EditMember() {
	...
	member, err := model.NewMember().Find(member_id)
	if err != nil {
		logs.Error(err)
		c.Abort("404")
	}
	if c.Ctx.Input.IsPost() {
		...
		if password1 != "" && member.AuthMethod != config.AuthMethodLDAP {
			member.Password = password1
		}
		...
		if password1 != "" {
			password, err := passutil.PasswordHash(password1)
			if err != nil {
				logs.Error(err)
...
func (c *ManagerController) DeleteMember() {
	...
	superMember, err := model.NewMember().FindByFieldFirst("role", 0)
	if err != nil {
		logs.Error(err)
		c.JsonResult(5001, "未能找到超级管理员")
	}

	err = model.NewMember().Delete(member_id, superMember.MemberId)
	if err != nil {
		logs.Error(err)
...
func (c *ManagerController) Books() {
	...
	for i, book := range books {
		books[i].Description = htmlutil.StripTags(string(blackfriday.Run([]byte(book.Description))))
...
func (c *ManagerController) EditBook() {
	...
	if c.Ctx.Input.IsPost() {
		...
		if !model.NewItemsets().Exist(itemId) {
...
func (c *ManagerController) CreateToken() {
	...
	if action == "create" {
		...
		book.PrivateToken = string(krand.Krand(config.GetTokenSize(), krand.KC_RAND_KIND_ALL))
		if err := book.Update(); err != nil {
			logs.Error("生成阅读令牌失败 => ", err)
			c.JsonResult(6003, i18n.Tr(c.Lang, "message.failed"))
		}
		c.JsonResult(0, "ok", config.URLFor("DocumentController.Index", ":key", book.Identify, "token", book.PrivateToken))
	} else {
		book.PrivateToken = ""
		if err := book.Update(); err != nil {
...
func (c *ManagerController) Transfer() {
	...
	rel, err := model.NewRelationship().FindFounder(book.BookId)
	...
	err = model.NewRelationship().Transfer(book.BookId, rel.MemberId, member.MemberId)
...
func (c *ManagerController) AttachList() {
	...
	for _, item := range attachList {
		...
		p := filepath.Join(config.WorkingDirectory, item.FilePath)
...
func (c *ManagerController) AttachDetailed() {
	...
	attach.FilePath = filepath.Join(config.WorkingDirectory, attach.FilePath)
	attach.HttpPath = config.URLForWithCdnImage(attach.HttpPath)
...
func (c *ManagerController) AttachDelete() {
	...
	attach, err := model.NewAttachment().Find(attachId)
...
func (c *ManagerController) LabelList() {
	...
	labels, totalCount, err := model.NewLabel().FindToPager(pageIndex, config.PageSize)
	if err != nil {
	...
	c.Data["TotalPages"] = int(math.Ceil(float64(totalCount) / float64(config.PageSize)))
...
func (c *ManagerController) LabelDelete() {
	...
	label, err := model.NewLabel().FindFirst("label_id", labelId)
	if err != nil {
...
func (c *ManagerController) Config() {
	...
	if c.Ctx.Input.IsPost() {
		...
		err = filetil.CopyFile(tf.Name(), config.ConfigurationFile)
		if err != nil {
	...
	if b, err := os.ReadFile(config.ConfigurationFile); err == nil {
...
func (c *ManagerController) Team() {
	...
	teams, totalCount, err := model.NewTeam().FindToPager(pageIndex, config.PageSize)
...
func (c *ManagerController) TeamEdit() {
	...
	team, err := model.NewTeam().First(teamId)
...
func (c *ManagerController) TeamDelete() {
	...
	err := model.NewTeam().Delete(teamId)
...
func (c *ManagerController) TeamMemberList() {
	...
	teams, totalCount, err := model.NewTeamMember().SetLang(c.Lang).FindToPager(teamId, pageIndex, config.PageSize)
	if err != nil && err != orm.ErrNoRows {
		c.ShowErrorPage(500, err.Error())
	}
	if err == orm.ErrNoRows || len(teams) <= 0 {
...
func (c *ManagerController) TeamSearchMember() {
	...
	searchResult, err := model.NewTeamMember().FindNotJoinMemberByAccount(teamId, keyword, 10)
...
func (c *ManagerController) TeamMemberAdd() {
	...
	if teamId <= 0 || memberId <= 0 || roleId <= 0 || roleId > int(config.BookObserver) {
		c.JsonResult(5001, i18n.Tr(c.Lang, "message.system_error"))
	}

	teamMember := model.NewTeamMember()
	teamMember.MemberId = memberId
	teamMember.TeamId = teamId
	teamMember.RoleId = config.BookRole(roleId)
...
func (c *ManagerController) TeamMemberDelete() {
	...
	teamMember, err := model.NewTeamMember().FindFirst(teamId, memberId)
...
func (c *ManagerController) TeamChangeMemberRole() {
	...
	roleId, _ := c.GetInt("roleId")
	teamId, _ := c.GetInt("teamId")
	if memberId <= 0 || roleId <= 0 || teamId <= 0 || roleId > int(config.BookObserver) {
		c.JsonResult(5001, i18n.Tr(c.Lang, "message.param_error"))
	}

	teamMember, err := model.NewTeamMember().ChangeRoleId(teamId, memberId, config.BookRole(roleId))
...
func (c *ManagerController) TeamBookList() {
	...
	teams, totalCount, err := model.NewTeamRelationship().FindToPager(teamId, pageIndex, config.PageSize)
...
func (c *ManagerController) TeamBookAdd() {
	...
	teamRel := model.NewTeamRelationship()
...
func (c *ManagerController) TeamSearchBook() {
	...
	searchResult, err := model.NewTeamRelationship().FindNotJoinBookByName(teamId, keyword, 10)
...
func (c *ManagerController) TeamBookDelete() {
	...
	err := model.NewTeamRelationship().Delete(teamRelationshipId)
...
func (c *ManagerController) Itemsets() {
	...
	items, totalCount, err := model.NewItemsets().FindToPager(pageIndex, config.PageSize)
...
func (c *ManagerController) ItemsetsEdit() {
	...
	var item *model.Itemsets
	var err error
	if itemId > 0 {
		if item, err = model.NewItemsets().First(itemId); err != nil {
			if err == orm.ErrNoRows {
...
func (c *ManagerController) ItemsetsDelete() {
	...
	if err := model.NewItemsets().Delete(itemId); err != nil {
		c.JsonResult(5001, err.Error())
	}
	c.JsonResult(0, "OK")
}
