...
func (item *Document) TableNameWithPrefix() string {
	return config.GetDatabasePrefix() + item.TableName()
}
...
func (item *Document) InsertOrUpdate(cols ...string) error {
	o := orm.NewOrm()
	item.DocumentName = htmlutil.StripTags(item.DocumentName)
	var err error
...
func (item *Document) Processor() *Document {
	if item.Release != "" {
		item.Release = htmlutil.SafetyProcessor(item.Release)
				...
				if err == nil && len(attachList) > 0 {
					...
					for _, attach := range attachList {
						if strings.HasPrefix(attach.HttpPath, "/") {
							attach.HttpPath = strings.TrimSuffix(config.BaseUrl, "/") + attach.HttpPath
						}
			...
			docQuery.Find("img").Each(func(i int, selection *goquery.Selection) {
				...
				if src, ok := selection.Attr("src"); ok {
					...
					if cdnimg != "" && strings.HasPrefix(src, "/uploads/") {
						selection.SetAttr("src", urlutil.JoinURI(cdnimg, src))
					}
