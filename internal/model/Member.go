...
type Member struct {
	...
	Role          config.SystemRole `orm:"column(role);type(int);default(1);index" json:"role"`
...
func (m *Member) Login(account string, password string) (*Member, error) {
	...
	case "":
	case "local":
		ok, err := passutil.PasswordVerify(member.Password, password)
		if ok && err == nil {
			m.ResolveRoleName()
			return member, nil
		}
...
func (m *Member) Add() error {
	...
	if ok, err := regexp.MatchString(config.RegexpAccount, m.Account); m.Account == "" || !ok || err != nil {
		return errors.New("账号只能由英文字母数字组成，且在3-50个字符")
	}
	if m.Email == "" {
		return errors.New("邮箱不能为空")
	}
	if ok, err := regexp.MatchString(config.RegexpEmail, m.Email); !ok || err != nil || m.Email == "" {
	...
	hash, err := passutil.PasswordHash(m.Password)
...
func (m *Member) ResolveRoleName() {
	if m.Role == config.MemberSuperRole {
		m.RoleName = i18n.Tr(m.Lang, "common.administrator")
	} else if m.Role == config.MemberAdminRole {
		m.RoleName = i18n.Tr(m.Lang, "common.editor")
	} else if m.Role == config.MemberGeneralRole {
...
func (m *Member) Valid(is_hash_password bool) error {
	...
	if m.Role != config.MemberGeneralRole && m.Role != config.MemberSuperRole && m.Role != config.MemberAdminRole {
