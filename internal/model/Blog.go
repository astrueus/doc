...
func (b *Blog) TableNameWithPrefix() string {
	return config.GetDatabasePrefix() + b.TableName()
}
...
func (b *Blog) Processor() *Blog {
	...
	b.BlogRelease = htmlutil.SafetyProcessor(b.BlogRelease)
		...
		content.Find("a").Each(func(i int, contentSelection *goquery.Selection) {
			if src, ok := contentSelection.Attr("href"); ok {
				if strings.HasPrefix(src, "http://") || strings.HasPrefix(src, "https://") {
					//logs.Info(src,config.BaseUrl,strings.HasPrefix(src,config.BaseUrl))
					if config.BaseUrl != "" && !strings.HasPrefix(src, config.BaseUrl) {
