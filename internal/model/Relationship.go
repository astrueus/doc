...
type Relationship struct {
	...
	RoleId config.BookRole `orm:"column(role_id);type(int)" json:"role_id"`
}
...
func (m *Relationship) UpdateRoleId(bookId, memberId int, roleId config.BookRole) (*Relationship, error) {
	o := orm.NewOrm()
	book := NewBook()
	book.BookId = bookId
	...
	if err == orm.ErrNoRows {
		...
		return m, err
	} else if m.RoleId == config.BookFounder {
...
func (m *Relationship) FindForRoleId(bookId, memberId int) (config.BookRole, error) {
	o := orm.NewOrm()
...
func (m *Relationship) Transfer(book_id, founder_id, receive_id int) error {
	...
	if founder.RoleId != config.BookFounder {
	...
	founder.RoleId = config.BookAdmin

	receive.MemberId = receive_id
	receive.RoleId = config.BookFounder
