...
> Round 1–2 ✅ · Round 3 MCP MVP ✅ · §十七 P0 ✅ · Round 4 代码主线 ✅ · **Round 5 🔶**（T12 已落地；T14/T16 待做；T6 与冻结项 → Round 6）。  
...
| [2.4.0-federated-auth/](./2.4.0-federated-auth/) | T16 联邦登录实施文档（先文档后代码） |
| [round-6/](./round-6/) | Round 6：Vite / 搜索 / Controller / 安全头 / bootstrap |
...
> **阅读顺序建议**：先看 [refactor-roadmap.md](./refactor-roadmap.md) → 已完成轮次看 [round-1-4/](./round-1-4/) → 进行中看 [round-5/](./round-5/) / [round-6/](./round-6/) → 接入 MCP 看 [mcp-integration.md](./mcp-integration.md)。AI 协作约定见仓库根目录 [AGENTS.md](../AGENTS.md)（含项目执行标准与 `.cursor/rules/` 索引）。
...
| Round 3 | MCP 10 工具 + HTTP Bearer | ✅ MVP | 搜索 FULLTEXT → R6 |
| Round 4 | 模型 / zap / i18n / 前端 P1 / Repo / 测试 | ✅ 主线 | 遗留移交 Round 5 / 6 |
| Round 5 | 工程化 / 对象存储 / 联邦登录 / kratos 向评估 | 🔶 | 见 [round-5](./round-5/)；T12 已落地；T14/T16 待做 |
| Round 6 | Vite / 搜索重定义 / 拆分债 | 📝 | 见 [round-6](./round-6/)；未开工 |

**Round 4 已合入（`v2.2.1`）要点：** BookModel 拆分、`md_` 修复、zap、Session 只存 id、前端 P1、`internal/i18n`、Repository 初版、pkg 单测、MCP P0。  
**Round 5 承接：** 缓存/ORM（kratos 向）报告、MCP P1、测试 CI、对象存储、OAuth2、`scripts`↔`deployments`、分层债与缓存实施。  
**Round 6 承接：** Vite P2、搜索重定义、Controller 拆分、安全头、拆 bootstrap。
