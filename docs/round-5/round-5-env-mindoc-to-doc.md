...
| （无旧名） | `DOC_SESSION_KEY` | `beegoserversessionkey`；example **无**公开默认值 |
...
| `MINDOC_DB_PASSWORD` | `DOC_DB_PASSWORD` | example **无**默认明文 |
| `MINDOC_DB_PREFIX` | `DOC_DB_PREFIX` | 表前缀默认仍为 `md_`（未改） |
| `MINDOC_ENABLE_MAIL` | `DOC_ENABLE_MAIL` | |
| `MINDOC_MAIL_NUMBER` | `DOC_MAIL_NUMBER` | |
| `MINDOC_SMTP_USER_NAME` | `DOC_SMTP_USER_NAME` | 示例邮箱改为 `admin@example.com` |
| `MINDOC_SMTP_HOST` | `DOC_SMTP_HOST` | 示例主机改为 `smtp.example.com` |
| `MINDOC_SMTP_PASSWORD` | `DOC_SMTP_PASSWORD` | |
| `MINDOC_SMTP_PORT` | `DOC_SMTP_PORT` | |
| `MINDOC_FORM_USERNAME` | `DOC_FORM_USERNAME` | 示例改为 `admin@example.com` |
...
| （无旧名） | `DOC_HTTP_LOGIN_SECRET` | `http_login_secret`；example 无明文 |
| `MINDOC_LDAP_PASSWORD` | `DOC_LDAP_PASSWORD` | example 无默认明文 |
