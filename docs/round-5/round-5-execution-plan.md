...
>
> **状态（2026-08-31 修订）：** 🔶 **批次 A + T5/T8/T9 已合入**；尾巴已收口（T7 云端 CI 绿）；**T12-a/b 已落地**（业务调用仍待 T12-c）。搜索 T3/T4 **持续冻结**。明细以 [§十四 追踪表](#十四追踪表) 为准。  
...
> **进度（2026-08-31）：** T12-a/b 已落地（Aside + Ristretto/Redis/Tag/PubSub/`Open`）；T12-c/d 业务接入未做。`RegisterCache()` 未改。
...
| T9 | Repo 扩面 + 可选 Service | | | ✅ 已实施（未建 service；Read 等 T12） |
| T10 | Controller 域拆分 | — | — | ⏸ **暂不拆**（解冻后勿平铺） |
| T11 | 安全头 middleware | | | ⏳ 可选 |
| T12 | 缓存完全重构实施 | | | 🔶 **T12-a/b 已落地**；c/d ⏳ |
...
- [ ] 若动缓存：新前缀生效；压测击穿/穿透；指标可看；可回滚 `mode`（T12-b 已提供 `Open`+conf；业务尚未切换）  
