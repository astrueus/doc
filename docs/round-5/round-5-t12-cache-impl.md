...
> **状态：** ✅ T12-a/b/c/d 已落地（2026-08-31）。  
> Document / Blog / MCP Token 均走 `GetOrLoad`。`beego_adapter.go` 仅留测试桩；启动不再初始化 beego 业务缓存。  
> 运维与压测：[round-5-t12-ops.md](./round-5-t12-ops.md)。
...
# T12-d：beego_adapter.go / 旧 Get/Set 仅留测试桩；RegisterCache 挂 NullCache
```

**T12-a/b/c/d 已落地（2026-08-31）：** 内核 + `Open` + Document/Blog/MCP Token 接入。`RegisterAside()` 在 bootstrap 调用；`RegisterCache()` 只挂 `NullCache`。

业务接入示例：

```text
internal/repository/document_repo.go   # Find 外包 GetOrLoad；Update 后 InvalidateTag
internal/repository/mcp_token.go       # ResolveAPIToken / InvalidateAPIToken
internal/mcp/http_auth.go              # Bearer → MemberRepo.ResolveAPIToken
```
...
T12-b **增量**写进现有 `[cache]`。Aside 前缀与遗留 beego 前缀分开。`cache_provider` 在 T12-d 后不再驱动业务缓存。
...
- `Open` 由 `RegisterAside()` 在 `cache=true` 时调用（默认 `cache_mode=local`）。`cache=false` 则文档/博客/Token 直打库。
...
| **T12-c** | Document / Blog 接入；model 内旧 cache 调用删除 | **2026-08-31 已完成**（`DocumentRepo`/`BlogRepo` + 写路径钩子失效） |
| **T12-d** | MCP Token 接入 + 压测脚本 / 文档 | **2026-08-31 已完成**（`MemberRepo.ResolveAPIToken`；`deployments/scripts/cache_load_test.*`；[round-5-t12-ops.md](./round-5-t12-ops.md)） |
...
- [x] 业务路径零 `beego/cache` 依赖（Session 除外）— **T12-d**（`RegisterCache` 仅 `NullCache`；适配留测试桩）  
- [x] Document / Blog 经 `GetOrLoad`；写后 Invalidate/Tag 有单测 — **T12-c**  
- [x] MCP Token 经 `GetOrLoad` — **T12-d**（`mcp_token_test.go`；吊销走 `InvalidateAPIToken`）  
- [x] 击穿 / 穿透 / Soft refresh / jitter 有自动化测试（`aside_test.go`；压测入口 `cache_load_test.sh` / `.ps1`）  
...
T12-c 已接 `RegisterAside()`。`cache=true` 时 Document / Blog / MCP Token 走新内核。验证仍以单测为准；手工 compose 可选。
...
- [round-5-t12-ops.md](./round-5-t12-ops.md)  
