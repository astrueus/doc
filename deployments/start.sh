...
if ! [ -f "/doc-sync-host/sync.sh" ]; then
    ...
    # 同步内容（Round 2: static/views 位于 web/ 下）
    # configs: 配置
    # database: sqlite方式数据库
    # runtime: 运行时数据(日志等)
    # web: 静态与模板（含 static/ views/）
    # uploads: 上传文件
    # echo "export SYNC_LIST='configs;database;runtime;web;uploads'" >> /doc-sync-host/sync.sh
    # echo "export SYNC_LIST=" >> /doc-sync-host/sync.sh
    echo "export SYNC_LIST='configs;database;uploads'" >> /doc-sync-host/sync.sh
